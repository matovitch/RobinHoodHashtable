...
struct Bucket
{
    ...
    constexpr static std::size_t EMPTY  = static_cast<std::size_t>(-1);

    void markEmpty() { _dib = EMPTY; }
    bool isEmpty() const { return _dib == EMPTY; }

    std::size_t _dib = EMPTY;   // Distance to Initial Bucket
 ...
 */
template <// TODO uncomment typename T,                               // type of the contained values
         ...
         > struct RobinHoodHashTable
{
    ...
    void insert(value_type t)
    {
        ...
        do
        {
            ...
            const std::size_t dibTmp = head->_dib;
    ...
    static const H          _hasher;
    std::size_t             _size;
    std::size_t             _capacity = INIT_SIZE;
};
